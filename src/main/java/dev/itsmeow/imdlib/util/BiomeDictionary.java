...
import static dev.itsmeow.imdlib.util.BiomeDictionary.Type.BEACH;
import static dev.itsmeow.imdlib.util.BiomeDictionary.Type.COLD;
import static dev.itsmeow.imdlib.util.BiomeDictionary.Type.CONIFEROUS;
import static dev.itsmeow.imdlib.util.BiomeDictionary.Type.DENSE;
import static dev.itsmeow.imdlib.util.BiomeDictionary.Type.DRY;
import static dev.itsmeow.imdlib.util.BiomeDictionary.Type.END;
import static dev.itsmeow.imdlib.util.BiomeDictionary.Type.FOREST;
import static dev.itsmeow.imdlib.util.BiomeDictionary.Type.HILLS;
import static dev.itsmeow.imdlib.util.BiomeDictionary.Type.HOT;
import static dev.itsmeow.imdlib.util.BiomeDictionary.Type.JUNGLE;
import static dev.itsmeow.imdlib.util.BiomeDictionary.Type.MESA;
import static dev.itsmeow.imdlib.util.BiomeDictionary.Type.MODIFIED;
import static dev.itsmeow.imdlib.util.BiomeDictionary.Type.MOUNTAIN;
import static dev.itsmeow.imdlib.util.BiomeDictionary.Type.MUSHROOM;
import static dev.itsmeow.imdlib.util.BiomeDictionary.Type.NETHER;
import static dev.itsmeow.imdlib.util.BiomeDictionary.Type.OCEAN;
import static dev.itsmeow.imdlib.util.BiomeDictionary.Type.OVERWORLD;
import static dev.itsmeow.imdlib.util.BiomeDictionary.Type.PLAINS;
import static dev.itsmeow.imdlib.util.BiomeDictionary.Type.PLATEAU;
import static dev.itsmeow.imdlib.util.BiomeDictionary.Type.RARE;
import static dev.itsmeow.imdlib.util.BiomeDictionary.Type.RIVER;
import static dev.itsmeow.imdlib.util.BiomeDictionary.Type.SANDY;
import static dev.itsmeow.imdlib.util.BiomeDictionary.Type.SAVANNA;
import static dev.itsmeow.imdlib.util.BiomeDictionary.Type.SNOWY;
import static dev.itsmeow.imdlib.util.BiomeDictionary.Type.SPARSE;
import static dev.itsmeow.imdlib.util.BiomeDictionary.Type.SPOOKY;
import static dev.itsmeow.imdlib.util.BiomeDictionary.Type.SWAMP;
import static dev.itsmeow.imdlib.util.BiomeDictionary.Type.VOID;
import static dev.itsmeow.imdlib.util.BiomeDictionary.Type.WASTELAND;
import static dev.itsmeow.imdlib.util.BiomeDictionary.Type.WET;
...
public class BiomeDictionary {
    ...
    public static final class Type {
        ...
        /*
         * Tags specifying the dimension a biome generates in. Specifying none implies a
         * biome that generates in a modded dimension
         */
     ...
     *
     */
    public static void addTypes(RegistryKey<Biome> biomeKey, Type... types) {
        Preconditions.checkArgument(ForgeRegistries.BIOMES.containsKey(biomeKey.getLocation()), "Cannot add types to unregistered biome %s", biomeKey);
        ...
        BiomeInfo biomeInfo = getBiomeInfo(biomeKey);
     ...
     */
    @Nonnull
    public static Set<Type> getTypes(RegistryKey<Biome> biomeKey) {
        ensureHasTypes(biomeKey);
        return getBiomeInfo(biomeKey).typesUn;
    }
     ...
     *
     */
    public static boolean hasType(RegistryKey<Biome> biomeKey, Type type) {
        return getTypes(biomeKey).contains(type);
    }
     ...
     *
     */
    public static boolean hasAnyType(RegistryKey<Biome> biomeKey) {
        return !getBiomeInfo(biomeKey).types.isEmpty();
    }

    /**
     * Automatically adds appropriate types to a given biome based on certain
     * heuristics. If a biome's types are requested and no types have been added to
     * the biome so far, the biome's types will be determined and added using this
     * method.
     *
     */
    public static void makeBestGuess(RegistryKey<Biome> biomeKey) {
        Preconditions.checkArgument(ForgeRegistries.BIOMES.containsKey(biomeKey.getLocation()), "Cannot guess types of unregistered biome %s", biomeKey);
        Biome biome = ForgeRegistries.BIOMES.getValue(biomeKey.getLocation());
        Type type = Type.fromVanilla(biome.getCategory());
        if(type != null) {
            BiomeDictionary.addTypes(biomeKey, type);
        }

        if(biome.getDownfall() > 0.85f) {
            BiomeDictionary.addTypes(biomeKey, WET);
        }

        if(biome.getDownfall() < 0.15f) {
            BiomeDictionary.addTypes(biomeKey, DRY);
        }

        if(biome.getTemperature() > 0.85f) {
            BiomeDictionary.addTypes(biomeKey, HOT);
        }

        if(biome.getTemperature() < 0.15f) {
            BiomeDictionary.addTypes(biomeKey, COLD);
        }

        if(biome.isHighHumidity() && biome.getDepth() < 0.0F && (biome.getScale() <= 0.3F && biome.getScale() >= 0.0F)) {
            BiomeDictionary.addTypes(biomeKey, SWAMP);
        }

        if(biome.getDepth() <= -0.5F) {
            if(biome.getScale() == 0.0F) {
                BiomeDictionary.addTypes(biomeKey, RIVER);
            } else {
                BiomeDictionary.addTypes(biomeKey, OCEAN);
            }
        }

        if(biome.getScale() >= 0.4F && biome.getScale() < 1.5F) {
            BiomeDictionary.addTypes(biomeKey, HILLS);
        }

        if(biome.getScale() >= 1.5F) {
            BiomeDictionary.addTypes(biomeKey, MOUNTAIN);
        }
    }

    // Internal implementation
    private static BiomeInfo getBiomeInfo(RegistryKey<Biome> biomeKey) {
        return biomeInfoMap.computeIfAbsent(biomeKey.getLocation(), k -> new BiomeInfo());
    }

    /**
     * Ensure that at least one type has been added to the given biome.
     */
    static void ensureHasTypes(RegistryKey<Biome> biomeKey) {
        if(!hasAnyType(biomeKey)) {
            makeBestGuess(biomeKey);
            LOGGER.warn("No types have been added to Biome {}, types have been assigned on a best-effort guess: {}", biomeKey.getLocation(), !getBiomeInfo(biomeKey).types.isEmpty() ? getBiomeInfo(biomeKey).types : "could not guess types");
        }
    }
